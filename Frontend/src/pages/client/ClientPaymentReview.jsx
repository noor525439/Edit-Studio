...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase border ${styles[variant]}`}>
      <Icon size={14} />
      {label}
    </span>
  );
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Payment Review</h1>
                <p className="text-sm text-slate-500">{order?.projectTitle}</p>
                {isAdminView && (
                  <p className="text-[10px] font-mono text-slate-400 mt-0.5">Order ID: {projectId}</p>
                )}
              </div>
            </div>
          </div>
